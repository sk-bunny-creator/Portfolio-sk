...
document.addEventListener('DOMContentLoaded', () => {
    const galaxyPachinkoGame = (p) => {
        ...
        // VOLUME FIX: Added a master volume multiplier for easy adjustment.
        const MASTER_VOLUME_MULTIPLIER = 4.0; // Adjust this value to set overall game volume. 1.0 is default.
        ...
        p.playPegHitSound = (isBoosterHit) => playSoundIfReady(() => {
            if (isBoosterHit) {
                ...
                sounds.boosterFilter.res(10);
                 // VOLUME FIX: Increased booster hit volume
                sounds.boosterEnv.play(sounds.boosterOsc, 0, 0.15 * MASTER_VOLUME_MULTIPLIER); 
                ...
                // VOLUME FIX: Increased peg hit volume
                sounds.pegHitEnv.play(sounds.pegHitOsc, 0, 0.1 * MASTER_VOLUME_MULTIPLIER); 
            }
        });
        p.playPegBreakSound = () => playSoundIfReady(() => {
            sounds.pegBreakFilter.freq(p.random(1500, 2500));
            sounds.pegBreakFilter.res(25); 
            // VOLUME FIX: Increased peg break volume
            sounds.pegBreakEnv.play(sounds.pegBreakNoise, 0, 0.25 * MASTER_VOLUME_MULTIPLIER); 
        });
        p.playWallHitSound = () => playSoundIfReady(() => {
            sounds.wallHitFilter.freq(p.random(100,180)); 
             // VOLUME FIX: Increased wall hit volume
            sounds.wallHitEnv.play(sounds.wallHitNoise, 0, 0.15 * MASTER_VOLUME_MULTIPLIER);
        });
        p.playWinSound = () => playSoundIfReady(() => {
            // VOLUME FIX: Simplified and increased base amplitude for win sound
            let baseAmp = 0.15 * MASTER_VOLUME_MULTIPLIER;
            ...
            shimmerFilter.res(8);     
            shimmerNoise.start();
            // VOLUME FIX: Increased shimmer volume
            shimmerEnv.play(shimmerNoise, 0, 0.1 * MASTER_VOLUME_MULTIPLIER); 
        ...
        p.playSlotLandSound = (isWinSlot) => {
            if (isWinSlot) {
                ...
                playSoundIfReady(() => {
                    ...
                    // VOLUME FIX: Increased slot land volume
                    sounds.slotLandEnv.play(sounds.slotLandOsc, 0, 0.2 * MASTER_VOLUME_MULTIPLIER); 
                });
            }
        };
        p.playBallReleaseSound = () => playSoundIfReady(() => {
            ...
            // VOLUME FIX: Increased ball release volume
            sounds.ballReleaseEnv.play(sounds.ballReleaseNoise, 0, 0.25 * MASTER_VOLUME_MULTIPLIER);
            sounds.ballReleaseFilter.freq(500, 0.2); 
        });
        p.playFizzleSound = () => playSoundIfReady(() => {
            sounds.fizzleOsc.freq(p.random(150, 250));
            // VOLUME FIX: Increased fizzle volume
            sounds.fizzleEnv.play(sounds.fizzleOsc, 0, 0.3 * MASTER_VOLUME_MULTIPLIER);
